"""
app.py — Gradio interface for RAG Research Assistant
Run locally : python app.py
HuggingFace : this file is the entry point (Space SDK: gradio)
"""
...
def _load_existing():
    """Try to restore index from disk on startup."""
    ...
    return "No existing index found. Upload PDFs to begin."
...
#citations-box {font-size: 13px; color: gray;}
.status-box {padding: 8px; border-radius: 6px;}
"""

with gr.Blocks(title="RAG Research Assistant", css=CSS, theme=gr.themes.Soft()) as demo:

    gr.Markdown("""
# RAG Research Assistant
**Local, offline, cited.** Upload research papers → ask questions → get answers with source citations.
> Powered by FAISS · HuggingFace Embeddings · Ollama (local LLM)
""")
    ...
    with gr.Row():
        ...
        with gr.Column(scale=1):
            ...
            ingest_status = gr.Textbox(label="Status", lines=2, elem_classes=["status-box"])
    ...
    ingest_btn.click(
        handle_upload,
        inputs=[file_input, append_toggle],
    ...
    ask_btn.click(
        handle_query,
        inputs=[question_input, top_k_slider, model_input],
    ...
    gr.Markdown("""
---
**Local setup:** `ollama serve` + `ollama pull llama3`  
**HuggingFace Space:** Set `OLLAMA_HOST` env var to your Ollama endpoint, or swap `generator.py` to use `InferenceClient`.
""")
